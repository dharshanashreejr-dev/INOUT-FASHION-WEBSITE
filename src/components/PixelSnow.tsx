  ...
  for (int i = 0; i < 96; i++) {
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        float dist = length(testUV);
        if (dist < flakeSize) {
          float flakeSizeRatio = uFlakeSize / flakeSize;
          float intensity = exp2(-(t + toIntersection) * invDepthFade) *
                           min(1.0, flakeSizeRatio * flakeSizeRatio) * uBrightness;
          float flakeAlpha = clamp(intensity * 1.6, 0.0, 1.0);
          gl_FragColor = vec4(uColor * pow(vec3(intensity), vec3(uGamma)), flakeAlpha);
          return;
        }
      }
    }
...
// Gold-tinted ambient particles — reskinned from "snow" to suit a premium clothing brand
export default function PixelSnow({
  ...
  return <div ref={containerRef} className={`pixel-snow-container ${className}`} />;
}
